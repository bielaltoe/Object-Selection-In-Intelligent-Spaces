from source.protobuf.ros_pb2 import ROSMessage
...
class SkeletonPosition:
    ...
    def send_to(self, msg):
        print("-"*100)
        print(f"[SkeletonPosition] raw message payload: {msg}")
        ros_message = self.get_ros_message(msg)
        message = Message(content=ros_message)
        message.topic = self.topic
        print(f"[SkeletonPosition] IS message envelope: {message}")
        print(f"[SkeletonPosition] publishing on channel: {self._channel}, topic: {self.topic}")
