...
def should_publish(track_id, new_coords, cache, threshold=0.1, timeout=10.0):
    ...
    last_track_id = cache[-1] if cache else None
    print(f"Cache inside should_publish: {cache}")
    print(f"Last track ID: {last_track_id}")
    if (track_id not in cache) or track_id != last_track_id:
